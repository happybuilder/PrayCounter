...
public class PrayCounterDbHelper extends SQLiteOpenHelper {
	...
	private String[] counter_sql = {
			// Version 1
			"create table counter(" + " _id integer primary key autoincrement"
					+ ", current integer not null default 0" // 已誦了多少遍經文.
	...
	private long initCounterTable(SQLiteDatabase db) {
		...
		values.put("notes", "");
		values.put("lastupdate", DateUtils.dateToString(new Date()));
